    ...
    async def run():
        async with websockets.connect(ws_url) as ws:
            ...
            res = await call("Runtime.evaluate", {
                "expression": """(() => {
                    const testPanels = ['panel-options', 'panel-news', 'panel-fundamentals', 'panel-other-factors', 'panel-reco'];
                    const results = {};
                    for (const pid of testPanels) {
                        const el = document.getElementById(pid);
                        if (!el) { results[pid] = 'not found'; continue; }
                        el.classList.add('active');
                        const style = window.getComputedStyle(el);
                        const childrenInfo = Array.from(el.children).map(c => ({
                            tag: c.tagName,
                            id: c.id,
                            cls: c.className,
