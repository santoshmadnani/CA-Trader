...
signed_cookie = signer.sign(base64.b64encode(json.dumps(session_dict).encode("utf-8"))).decode("utf-8")
...
inds = r_chart.get("technical", {}).get("technical", {}).get("indicators", [])
print(f"Count of indicators: {len(inds)}")
for i in inds:
    print(f"  {i.get('name')}: {i.get('value')} ({i.get('signal')}) - {i.get('criteria')}")
"""
