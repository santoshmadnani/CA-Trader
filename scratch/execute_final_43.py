#!/usr/bin/env python3
"""Build and verify flawless Dashboard release from pristine 42.zip base."""
...
# 1. Fix sidebar max-width in CSS to prevent layout distortion
html = html.replace(
...
# 2. Remove Auto Trade filter checkbox from sidebar
html = re.sub(
...
# 3. Update Dow Jones placeholder in HTML
html = re.sub(
...
# 4. Remove section bar auto-slide
html = html.replace(
...
# 5. Extract chunks to move from panel-charts
p_summary = html.find('id="masterSummaryCard"')
p_shell = html.find('id="chartShell"')
chunk1_summary_banner_evidence = html[html.rfind('<div', 0, p_summary):html.rfind('<div', 0, p_shell)]

p_greeks = html.find('id="chartGreeksGrid"')
p_sim = html.find('id="chartPriceSensitivityCard"')
p_candlepats = html.find('id="candlePatternCardTitle"')
p_candlepats_card = html.rfind('<div class="card"', 0, p_candlepats)
chunk2_greeks_and_sim = html[html.rfind('<div class="grid', 0, p_greeks):p_candlepats_card]

# Extract recommendation banner from chunk1
banner_match = re.search(r'(<div class="card" id="chartRecoBanner"[\s\S]*?</div>\s*</div>\s*</div>)', chunk1_summary_banner_evidence)
reco_banner_html = banner_match.group(1) if banner_match else ''

# Extract simulator card from chunk2
sim_match = re.search(r'(<div class="card" style="margin-bottom:14px;" id="chartPriceSensitivityCard"[\s\S]*?</div>\s*</div>\s*</div>)', chunk2_greeks_and_sim)
sim_card_html = sim_match.group(1) if sim_match else ''

# Remove chunk1 and chunk2 from panel-charts
html = html.replace(chunk1_summary_banner_evidence, '')
html = html.replace(chunk2_greeks_and_sim, '')
print("+ Cleaned panel-charts (moved to Dashboard)")

# 6. Build Master Confluence Table HTML (Requirement 4 & 5: All indicators, all patterns, all news, all other factors, all Greeks, in one table, NO boring sentences!)
...
# 7. Build Complete Dashboard Panel HTML
...
# 8. Option Chain Expiry Selector in Options Toolbar
html = html.replace(
...
# 9. Replace showTab to cleanly handle dashboard and other tabs without blank data
...
# 10. Recommendation History Table Head & Row: add Outcome Status column
old_rec_head = """              <tr>
...
# 11. Javascript implementations for loadDashboard and updateDashboardConfluenceTable
dashboard_js = """
  // ==========================================
