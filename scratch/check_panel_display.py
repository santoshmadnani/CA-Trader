        ...
        async with websockets.connect(ws_url) as ws:
            ...
            for t in ['dashboard', 'charts', 'options', 'news', 'fundamentals', 'movers', 'other-factors', 'orders', 'funds']:
                res = await call('Runtime.evaluate', {
                    'expression': f'(() => {{ showTab("{t}"); const p = document.getElementById("panel-{t}"); return p ? window.getComputedStyle(p).display : "null"; }})()'
                })
                disp = res.get('result',{}).get('result',{}).get('value')
                res2 = await call('Runtime.evaluate', {
                    'expression': f'(() => {{ const p = document.getElementById("panel-{t}"); return p ? p.scrollHeight : 0; }})()'
                })
                sh = res2.get('result',{}).get('result',{}).get('value')
                print(f'Tab {t}: display = {disp}, scrollHeight = {sh}')
