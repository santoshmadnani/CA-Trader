# -*- coding: utf-8 -*-
"""Full surgical patch for Release 45 in terminal.html."""
import re
...
# 2. Add CSS for .wl-r-btn and .compact-date-input
r_btn_css = """
...
# Find beginning of card div for chartPriceSensitivityCard
start_sim = code.rfind('<div class="card"', 0, idx_sim)
# Find beginning of card div for dashHistoryCard
...
# 14. Option Chain: Cache and avoid 429 when toggling views (Item 11)
...
# Extract renderZerodhaOptionChain from fetchOptionChain
old_foc_start = """  async function fetchOptionChain(){
...
new_foc_start = """  function renderZerodhaOptionChain(d){
    const host = document.getElementById('zerodhaOptionChainContainer') || document.getElementById('optionChainTable');
    if(!host || !d) return;
    try{
      const underSym = extractUnderlying(S);
...
# We'll adapt fetchOptionChain to call renderZerodhaOptionChain(d)
code = code.replace(old_foc_start, new_foc_start, 1)
# And replace the end of fetchOptionChain with call
foc_call_glue = """
  async function fetchOptionChain(){
    ...
    try{
      ...
      APP_CACHE.options = d;
      optionState.chain = d;
      renderZerodhaOptionChain(d);
    }catch(err){
      if(optionState.chain) renderZerodhaOptionChain(optionState.chain);
      else if(host) host.innerHTML = `<div class="options-empty">Option chain unavailable (${err.message}). Cached fallback active.</div>`;
    }
  }
"""
# Insert after renderZerodhaOptionChain closing brace
rz_end_target = "window.renderZerodhaOptionChain = renderZerodhaOptionChain;"
if rz_end_target not in code:
    # insert before updateBacktestOptionHeader or similar
    code = code.replace("window.showGreeks = showGreeks;", foc_call_glue + "\n  window.showGreeks = showGreeks;\n  window.renderZerodhaOptionChain = renderZerodhaOptionChain;", 1)
print("14b. Extracted renderZerodhaOptionChain function")
