...
try:
    ...
    async def run():
        async with websockets.connect(ws_url) as ws:
            ...
            # Test direct call to updateDashboardConfluenceTable
            res = await call("Runtime.evaluate", {
                "expression": """
                (() => {
                    if (typeof window.updateDashboardConfluenceTable !== 'function') return { error: 'fn missing' };
                    window.updateDashboardConfluenceTable(true, 23118.6, 'NIFTY');
                    const rows = Array.from(document.querySelectorAll('#dashConfluenceTableBody tr')).map(r => r.textContent.trim());
                    return {
            ...
            sim_res = await call("Runtime.evaluate", {
                ...
                (() => {
                    ...
                    return {
                        ...
                        total: document.getElementById('chartSimTotalImpact')?.textContent,
