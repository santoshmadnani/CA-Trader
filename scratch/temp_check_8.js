...
(() => {
  ...
  function renderIndicatorList(filterText = ''){
    ...
    container.innerHTML = filtered.map(ind => {
      const isApplied = (state.indicators || []).some(x => x.name === ind);
      return `
    ...
    container.querySelectorAll('.indicator-row-item').forEach(row => {
      ...
      row.addEventListener('click', () => {
        ...
        if(!indName) return;
        const existsIdx = (state.indicators || []).findIndex(x => x.name === indName);
        if(existsIdx >= 0){
          state.indicators.splice(existsIdx, 1);
          toast(`Removed ${indName} from chart`);
        } else {
          state.indicators.push({ name: indName, params: '14', color: '#26d9a6' });
  ...
  function updateAtUiForSymbol(sym){
    sym = sym || S || (typeof selectedSymbol === 'function' ? selectedSymbol() : 'NIFTY');
    if(!sym) return;
  ...
  async function syncMasterAtState(){
    const sym = S || (typeof selectedSymbol === 'function' ? selectedSymbol() : 'NIFTY');
  ...
  const criteriaBtn = document.getElementById('chartRecoCriteriaBtn');
  const criteriaModal = document.getElementById('chartRecoCriteriaModal');
  criteriaBtn?.addEventListener('click', (e) => {
    e.stopPropagation();
    if(criteriaModal){
      criteriaModal.style.display = (criteriaModal.style.display === 'none' || !criteriaModal.style.display) ? 'block' : 'none';
    }
  });
  document.getElementById('closeCriteriaModalBtn')?.addEventListener('click', () => {
  ...
  document.getElementById('saveCriteriaBtn')?.addEventListener('click', async () => {
    const p = Number(document.getElementById('chartCriteriaDesiredProfit')?.value || 500);
    const l = Number(document.getElementById('chartCriteriaMaxLoss')?.value || 1000);
    const c = Number(document.getElementById('chartCriteriaCapital')?.value || 50000);
    localStorage.setItem('ca_reco_profit', String(p));
    localStorage.setItem('ca_reco_loss', String(l));
    localStorage.setItem('ca_reco_capital', String(c));
    if(criteriaModal) criteriaModal.style.display = 'none';
    toast('Parameters updated! Refreshing recommendation…');
    if(typeof updateChartRecoBanner === 'function') await updateChartRecoBanner(null, S, true);
  });
  ...
  function updateBacktestSymbols(){
    ...
    const currSym = (typeof selectedSymbol === 'function' ? selectedSymbol() : S) || 'RELIANCE';
