...
(async()=>{
        ...
        if(q.p1!=null && q.p2!=null){
          [{px:x1,py:y1,pr:q.p1,isStart:true},{px:x2,py:y2,pr:q.p2,isStart:false}].forEach(pt=>{
            if(pt.px>=pad.l-10 && pt.px<=w-pad.r+10 && pt.py>=pad.t-10 && pt.py<=h-pad.b+10){
              x.beginPath();x.arc(pt.px,pt.py,isSelected?5:3.5,0,Math.PI*2);
              x.fillStyle=isSelected ? '#00e5ff' : (q.color||css('--gold'));
              x.fill();
  ...
  function bindPatternClicks(){
    ...
    document.querySelectorAll('[data-focus-pattern]').forEach(el=>el.onclick=async()=>{
      ...
      toast(`✦ Highlighted ${p.pattern} (${formatPatternTimeRange(p)}) — Auto-clears in 10s`);
    });

    // Also bind Chart Patterns (Double Top, Double Bottom, Head & Shoulders)
    document.querySelectorAll('[data-focus-chart-pattern]').forEach(el => el.onclick = async () => {
      ...
      if(!p) return;
      let startIdx = p.start_index != null ? p.start_index : 0;
      let endIdx = p.end_index != null ? p.end_index : ((state.candles || []).length - 1);
      ...
      toast(`✦ Highlighted ${p.pattern} on chart — Auto-clears in 10s`);
    });
  }
...
function applyDrawingPoint(x,y){const r=vp.getBoundingClientRect(),pt=chartPoint({clientX:r.left+x,clientY:r.top+y});const name=state.pendingDrawing;state.drawingStage.push(pt);const stage=state.drawingStage.length;const need=name==='Horizontal Line'||name==='Vertical Line'?1:name==='Risk / Reward'?3:name==='Parallel Channel'?3:2;if(stage<need){document.getElementById('drawingStatus').textContent=stage===1?'Click again to set the second point.':`Click ${need-stage} more time(s) to finish.`;return}const color=state.pendingColor||css('--gold');let d={name,color,type:''};if(name==='Horizontal Line')d={...d,type:'h',price:pt.price};else if(name==='Vertical Line')d={...d,type:'v',i:pt.i};else if(name==='Trend Line')d={...d,type:'trend_ray',i1:state.drawingStage[0].i,i2:pt.i,p1:state.drawingStage[0].price,p2:pt.price,infinite:true};else if(name==='Ray')d={...d,type:'ray',i1:state.drawingStage[0].i,i2:pt.i,p1:state.drawingStage[0].price,p2:pt.price};else if(name==='Arrow')d={...d,type:'arrow',i1:state.drawingStage[0].i,i2:pt.i,p1:state.drawingStage[0].price,p2:pt.price};else if(name==='Rectangle'||name==='Price Range')d={...d,type:name==='Rectangle'?'rect':'range',i1:state.drawingStage[0].i,i2:pt.i,p1:state.drawingStage[0].price,p2:pt.price,fill:name==='Rectangle'?`${color}22`:'transparent'};else if(name==='Fibonacci Retracement')d={...d,type:'fib',i1:state.drawingStage[0].i,i2:pt.i,p1:state.drawingStage[0].price,p2:pt.price};else if(name==='Risk / Reward'){d={...d,type:'rr',i:state.drawingStage[0].i,entry:state.drawingStage[0].price,stop:state.drawingStage[1].price,target:pt.price,fill:'transparent'}}else if(name==='Parallel Channel'){const a=state.drawingStage[0],b=state.drawingStage[1],c3=pt;const delta=c3.price-(a.price+(b.price-a.price));d={...d,type:'line',i1:a.i,i2:b.i,p1:a.price,p2:b.price,channelOffset:delta}}state.drawings.push(d);state.pendingDrawing=null;state.pendingColor=null;state.drawingStage=[];state.panArmed=false;state.yPanArmed=false;state.interactionMode='crosshair';vp.classList.remove('pan-mode','drawing-hover');const modeBtn=document.getElementById('chartModeToggle');if(modeBtn){modeBtn.textContent='Crosshair';modeBtn.classList.remove('active')}document.getElementById('drawingStatus').style.display='none';renderApplied();draw()}
  ...
  async function fetchOptionChain(){
    ...
    try{
      ...
      const lot = 15 if ("BANK" in S) else 25 if ("NIFTY" in S) else 100 if ("CRUDE" in S) else 1;
            ...
            ${rows.map(r => {
              ...
              return `
                <tr class="opt-chain-row" style="border-bottom:1px solid var(--border-soft);${isAtm ? 'background:rgba(59,130,246,0.06);' : ''}">
                  ${isGreeks ? `
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(c.delta)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(c.gamma)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(c.theta)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(c.iv)}</td>
                    <td style="padding:5px 8px;text-align:right;position:relative;">
                      <div style="display:flex;align-items:center;justify-content:flex-end;gap:5px;">
                        ...
                        <b class="cell-up" style="font-family:var(--font-mono);">₹${fmt(c.ltp)}</b>
                      </div>
                    </td>
                  ` : `
                    <td style="padding:5px 8px;font-family:var(--font-mono);position:relative;">
                    ...
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(c.volume)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(c.iv)}</td>
                    <td style="padding:5px 8px;text-align:right;position:relative;">
                      <div style="display:flex;align-items:center;justify-content:flex-end;gap:5px;">
                        <button type="button" class="opt-bs-btn buy" data-opt-buy="${esc(callSym)}" data-opt-ltp="${c.ltp||0}" title="Buy ${callSym}">B</button>
                  ...
                  <td style="text-align:center;padding:5px 8px;font-family:var(--font-mono);font-weight:700;">
                  ...
                  ${isGreeks ? `
                    <td style="padding:5px 8px;text-align:left;position:relative;">
                      <div style="display:flex;align-items:center;justify-content:flex-start;gap:5px;">
                        <b class="cell-down" style="font-family:var(--font-mono);">₹${fmt(p.ltp)}</b>
                    ...
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(p.iv)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(p.theta)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(p.gamma)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(p.delta)}</td>
                  ` : `
                    <td style="padding:5px 8px;text-align:left;position:relative;">
                      <div style="display:flex;align-items:center;justify-content:flex-start;gap:5px;">
                        <b class="cell-down" style="font-family:var(--font-mono);">₹${fmt(p.ltp)}</b>
                    ...
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(p.iv)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);">${fmt(p.volume)}</td>
                    <td style="padding:5px 8px;font-family:var(--font-mono);position:relative;">
      ...
      // Wire B/S Quick Order Buttons in Option Chain
