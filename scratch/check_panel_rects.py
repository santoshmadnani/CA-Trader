...
try:
    ...
    async def run():
        async with websockets.connect(ws_url) as ws:
            ...
            for t in ['charts', 'options', 'news', 'fundamentals', 'movers', 'other-factors', 'orders', 'funds']:
                res = await call('Runtime.evaluate', {
                    'expression': f'''(() => {{
                        ...
                        return "class=" + p.className + ", disp=" + window.getComputedStyle(p).display + ", w=" + rect.width + ", h=" + rect.height;
                    }})()'''
                })
